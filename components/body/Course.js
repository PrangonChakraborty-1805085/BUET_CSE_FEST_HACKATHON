  ...
  const handleEnroll=async(e)=>{
    try {
      ...
      if (ethereum) {
        ...
        // no need of checking if there is enough ethers
        var res = await  dummyContract.functionName(price);
        await res.wait(1);
       // const result = await dummyContract.functionName();
  ...
  return (
    <div class="lg:w-1/4 md:w-1/2 p-4 w-full">
      ...
      <div class="mt-4">
        ...
        <button class="flex mx-auto mt-6 text-white bg-indigo-500 border-0 py-2 px-5 focus:outline-none hover:bg-indigo-600 rounded" onClick={handleEnroll} disabled={disabled}>Enroll Now</button>
      </div>
    </div>
  );
}
